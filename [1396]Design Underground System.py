...
class UndergroundSystem(object):
    ...
    def __init__(self):
        ...
        self.avg = collections.defaultdict(list)
    ...
    def checkOut(self, id, stationName, t):
        ...
        :rtype: None
        """
        startStation, st = self.user[id]
        if (startStation, stationName) not in self.avg:
            self.avg[(startStation, stationName)] = (t - st, 1)
        else:
            avg, n = self.avg[(startStation, stationName)]
            self.avg[(startStation, stationName)] = ((avg * n + t - st) * 1.0 / (n + 1), n + 1)
