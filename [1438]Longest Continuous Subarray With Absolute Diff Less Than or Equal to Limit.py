...
class Solution(object):
    def longestSubarray(self, nums, limit):
        ...
        for j, n in enumerate(nums):
            ...
            bg.append(n)
            sm.append(n)
            while bg and sm and bg[0] - sm[0] > limit:
                if bg[0] == nums[i]: bg.popleft()
                if sm[0] == nums[i]: sm.popleft()
                i += 1
            res = max(res, j - i + 1)
        return res
print(Solution().longestSubarray([2,2, 4, 4], 0))
